...
int main(int argc, char **argv)
{
    ...
    if (m_pSDK->EnablePointProcessor())
    {
        ...
        m_pSDK->RegistPointCloudCallback([&points_count, &points_queue, &mutex_points](double time, cv::Mat points) {
            if (!points.empty())
            {
                {
    ...
    LOG(INFO) << "Time beg: " << times::to_local_string(time_beg) << ", end: " << times::to_local_string(time_end)
              << ", cost: " << elapsed_ms << "ms";
    LOG(INFO) << "depth count: " << points_count << ", fps: " << (1000.f * points_count / elapsed_ms);
#endif
    return 0;
}