...
int main(int argc, char **argv)
{
    ...
    m_pSDK->RegistImgCallback(
        [&last_img_time, &img_count, &image_queue, &mutex_image](double time, cv::Mat left, cv::Mat right) {
            if (!left.empty() && !right.empty())
            {
                ...
                if (last_img_time >= 0)
                {
                    ...
                    ss << std::fixed << "time stamp: " << std::setprecision(2) << time << ", fps: " << fps;
                    cv::putText(img, ss.str(), cv::Point(25, 25), FONT_FACE, FONT_SCALE, FONT_COLOR, THICKNESS);
                }
                last_img_time = time;
                {
