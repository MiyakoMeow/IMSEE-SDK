...
int main(int argc, char **argv)
{
    ...
    {
        m_pSDK->RegistDepthCallback([&depth_count, &depth_queue, &mutex_depth](double time, cv::Mat depth) {
            if (!depth.empty())
            {
    ...
    LOG(INFO) << "Time beg: " << times::to_local_string(time_beg) << ", end: " << times::to_local_string(time_end)
              << ", cost: " << elapsed_ms << "ms";
    LOG(INFO) << "depth count: " << depth_count << ", fps: " << (1000.f * depth_count / elapsed_ms);
#endif
    return 0;
}