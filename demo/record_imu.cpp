...
int main(int argc, char **argv)
{
    ...
    std::thread record_thread([&]() {
        ...
        ofs << "timestamp, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z" << std::endl;
        while (!is_stop || !imu_queue.empty())
        {
            if (!imu_queue.empty())
            {
                ...
                imu = imu_queue.front();
                ofs << imu.timestamp << ", " << imu.accel[0] << ", " << imu.accel[1] << ", " << imu.accel[2] << ", "
    ...
    m_pSDK->RegistImgCallback([&img_count, &image_queue, &mutex_image](double time, cv::Mat left, cv::Mat right) {
    ...
    LOG(INFO) << "Time beg: " << times::to_local_string(time_beg) << ", end: " << times::to_local_string(time_end)
              << ", cost: " << elapsed_ms << "ms";
    LOG(INFO) << "Img count: " << img_count << ", fps: " << (1000.f * img_count / elapsed_ms);
    LOG(INFO) << "Imu count: " << imu_count << ", hz: " << (1000.f * imu_count / elapsed_ms);
#endif
    return 0;
}