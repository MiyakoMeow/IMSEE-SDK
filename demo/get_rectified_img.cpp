...
int main(int argc, char **argv)
{
    ...
    if (m_pSDK->EnableRectifyProcessor())
    {
        m_pSDK->RegistRectifiedImgCallback(
            [&rectified_img_count, &rectified_queue, &mutex_rectified](double time, cv::Mat left, cv::Mat right) {
    ...
    LOG(INFO) << "Time beg: " << times::to_local_string(time_beg) << ", end: " << times::to_local_string(time_end)
              << ", cost: " << elapsed_ms << "ms";
    LOG(INFO) << "rectified count: " << rectified_img_count << ", fps: " << (1000.f * rectified_img_count / elapsed_ms);
#endif
    return 0;
}