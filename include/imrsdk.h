...
//                                               void *pParam);
using ModuleImageCallback = std::function<void(double time, unsigned char *pLeft, unsigned char *pRight, int width,
                                               int height, int channel, void *pParam)>;
/**
 * \~chinese \brief 图像数据回调函数
 */
// typedef void(CALL_METHOD *ImgCallback)(double time, cv::Mat left,
//                                       cv::Mat right);
using ImgCallback = std::function<void(double time, cv::Mat left, cv::Mat right)>;
/**
 * \~chinese \brief 矫正图回调函数
 */
// typedef void(CALL_METHOD *RectifiedImgCallback)(double time, cv::Mat left,
//                                                cv::Mat right);
using RectifiedImgCallback = std::function<void(double time, cv::Mat left, cv::Mat right)>;
/**
